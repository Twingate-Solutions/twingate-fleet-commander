...
| `connector_nofile` | `1024 <= int <= 1048576` | `131072` | Per-connector open-file-descriptor limit (`nofile`), stamped by the actuator on every Connector (Docker `Ulimits` / ECS `ulimits`). ~8 FDs per client tunnel, so this caps concurrent connections per Connector — a ceiling FC's CPU/throughput triggers cannot see, hence set explicitly rather than inherited from the host daemon default. **Not enforceable on the ACI backend** (Azure Container Instances exposes no ulimit control). |
| `connector_ephemeral_port_range` | `str "<low> <high>"`, `1024 <= low < high <= 65535` | `"10240 65535"` | Per-connector outbound source-port range, stamped as the `net.ipv4.ip_local_port_range` sysctl. A busy connector exhausts the default ~28k range and fails new connections — another ceiling FC's triggers can't see. **Docker backend only** (ECS/ACI use platform defaults). |
| `connector_log_max_size` | `str` (Docker size, e.g. `20m`, `1g`) | `"20m"` | Per-connector `json-file` log `max-size`. Connectors emit continuous ANALYTICS lines (always on), so without a cap their logs grow unbounded and can fill the host disk. Pinning `json-file` also keeps the file-reading log-shipper working. **Docker backend only** (ECS uses awslogs; ACI uses Azure Monitor). |
| `connector_log_max_file` | `int >= 1` | `5` | Per-connector `json-file` log `max-file` (rotated file count). Paired with `connector_log_max_size`. **Docker backend only.** |
